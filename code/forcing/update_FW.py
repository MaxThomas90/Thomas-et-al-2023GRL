#' % Update NEMO Antarctic freshwater forcing
#' % Max Thomas

#' # Summary
#' Here we change the values in the forcing file:
#'      eORCA1_runoff_v2.2x.nc
#' Specifically, we are changing the sofwisf variable, which gives the freshwater forcing from the ice shelf fronts.
...
data_dir = home_dir + '/configurations/forcing/runoff_ancils/'
...
print('----------------------------------------------------')
print('Updating freshwater for ' + os.sys.argv[1])
...
def intensive2extensive(intensive_values, areas, rescaler = 1.):
    ...
    if not( type(rescaler) is float):
        ...
        print('Sum of rescaled extensive = ' + str(kgs2Gtyr(extensive_values.sum())) + ' Gt/yr')
...
def kgs2Gtyr(kgs):
    gtyr = ( 1/( 1000 * 10**9 ) ) * 360*24*60*60 * kgs # Converts kg/s to Gt/yr
    return gtyr
...
def calculate_scaler(intensive_values, areas, logical_selector, F_scaled, selection_name='this selection'):
    ...
    F_changed = kgs2Gtyr( vex[logical_selector].sum() )
    F_unchanged = kgs2Gtyr( vex[np.invert(logical_selector)].sum() )
...
print('----------------------------------------------------')
def update_FWF(filename='eORCA1_runoff_v2.2x.nc',
    ...
    zvars_in = data_in.sofwfisf[month,:,:]
    ...
    # development note: in future (<, >) should be replaced with (<=,>) or (<,>=)
    # not an issue for hadgem3-gc3.1 N96 as there are no integer longitudes matching our regions
    ...
    extensive_0 = kgs2Gtyr(intensive2extensive(zvars_in.values,areas,rescaler=1.))
    extensive_new = kgs2Gtyr(intensive2extensive(zvars_out.values,areas,rescaler=1.))
    extensive_diff = np.subtract(extensive_new, extensive_0)
    
    fig = plt.figure(figsize=(20,10))
    
    ax1 = AA_axes(1, 'Freshwater input, default, intensive')
    im1 = ax1.scatter(x,y,c=intensive_0,s=intensive_0*10**5,cmap='Blues',transform=ccrs.PlateCarree())

    ax2 = AA_axes(2, 'Freshwater input, '+selection_name+', F_scaled = '+str(F_scaled) + ' Gt/yr, intensive')
    im2 = ax2.scatter(x,y,c=intensive_new,s=intensive_new*10**5,cmap='Blues',transform=ccrs.PlateCarree())
    
    ax3 = AA_axes(3, '$F_\mathrm{FW, 0} - F_\mathrm{FW, new}$, intensive')
    im3 = ax3.scatter(x,y,c=intensive_diff,s=intensive_diff*10**5,cmap='Blues',transform=ccrs.PlateCarree())
   ...
   #     fn_out = 'eORCA1_runoff_v2.2x_' + selection_name + str(int(F_scaled))
    fn_out = 'eORCA1_runoff_v2.2x_' + selection_name    

    data_out = data_in.copy()
    for mn in range(12):
        data_out.sofwfisf.values[mn,:,:] = zvars_out.values
